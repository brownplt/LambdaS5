...
let clean_env (exp : exp) : exp =
  let rec clean_rec ~in_usr_code (e : exp) (env : env) (used_ids : IdSet.t) : (exp * IdSet.t) = 
    let rec handle_option (opt : exp option) env used_ids : exp option * IdSet.t = 
      match opt with
      | Some (e) -> 
        let new_e, used_ids = clean_rec ~in_usr_code e env used_ids in
        Some (new_e), used_ids
      | None -> None, used_ids
    in 
    match e with
    ...
    | Object (p, attrs, strprop) ->
      ...
      let handle_prop (p : 'a) env used_ids : ('a * IdSet.t) = match p with
        | (s, Data(data, enum, config)) ->
          let value, used_ids = clean_rec ~in_usr_code data.value env used_ids in
          (s, Data({value = value; writable = data.writable}, 
                   enum, config)), used_ids
        | (s, Accessor (acc, enum, config)) ->
          let getter, used_ids = clean_rec ~in_usr_code acc.getter env used_ids in
          let setter, used_ids = clean_rec ~in_usr_code acc.setter env used_ids in
    ...
    | GetAttr (p, pattr, obj, field) ->
      let o, used_ids= clean_rec ~in_usr_code obj env used_ids in
      let fld, used_ids = clean_rec ~in_usr_code field env used_ids in
    ...
    | SetAttr (p, attr, obj, field, newval) -> 
      if not in_usr_code && useless_obj_set obj field env used_ids then begin
        ...
        let o, used_ids = clean_rec ~in_usr_code obj env used_ids in
        let f, used_ids = clean_rec ~in_usr_code field env used_ids in
        let v, used_ids = clean_rec ~in_usr_code newval env used_ids in
    ...
    | SetObjAttr (p, oattr, obj, attre) ->
      let o, used_ids = clean_rec ~in_usr_code obj env used_ids in
      let attr, used_ids = clean_rec ~in_usr_code attre env used_ids in
      SetObjAttr (p, oattr, o, attr), used_ids

    | GetField (p, obj, fld, args) ->
      let o, used_ids = clean_rec ~in_usr_code obj env used_ids in
      let f, used_ids = clean_rec ~in_usr_code fld env used_ids in
      let a, used_ids = clean_rec ~in_usr_code args env used_ids in
    ...
    | SetBang (p, x, x_v) ->
      let x_v, used_ids = clean_rec ~in_usr_code x_v env used_ids in
      let used_ids = IdSet.add x used_ids in
      dprint (sprintf "find usage point at SetBang %s\n" x);
      SetBang (p, x, x_v), used_ids

    | Op1 (p, op, e) ->
      let e, used_ids = clean_rec ~in_usr_code e env used_ids in
      Op1 (p, op, e), used_ids

    | Op2 (p, op, e1, e2) ->
      let e1, used_ids = clean_rec ~in_usr_code e1 env used_ids in
      let e2, used_ids = clean_rec ~in_usr_code e2 env used_ids in
      Op2 (p, op, e1, e2), used_ids

    | If (p, cond, thn, els) -> (* more optimization in constant folding *)
      let cond, used_ids = clean_rec ~in_usr_code cond env used_ids in
      let thn, used_ids = clean_rec ~in_usr_code thn env used_ids in
      let els, used_ids = clean_rec ~in_usr_code els env used_ids in
    ...
    | App (p, f, args) ->
      ...
      if not in_usr_code && useless_def_point f args env used_ids then begin
        ...
        let f, used_ids = clean_rec ~in_usr_code f env used_ids in
        let rec handle_args args env used_ids = match args with
          | [] -> args, used_ids
          | fst :: rest ->
            let v, used_ids = clean_rec ~in_usr_code fst env used_ids in
    ...
    | Seq (p0, Hint (p1, s, e0), e1) when EU.is_env_delimiter s ->
      let newe, used_ids = clean_rec ~in_usr_code:true e1 env used_ids in
      Seq (p0, Hint (p1, s, e0), newe), used_ids
      
    | Seq (p, e1, e2) ->
      let new_e2, e2_used_ids = clean_rec ~in_usr_code e2 env used_ids in
      let new_e1, e1_used_ids = clean_rec ~in_usr_code e1 env e2_used_ids in
    ...
    | Let (p, x, x_v, body) ->
      ...
      let new_body, used_ids = clean_rec ~in_usr_code body new_env used_ids in
    ...
    | TryCatch (p, body, catch) ->
      let b, used_ids = clean_rec ~in_usr_code body env used_ids in
      let c, used_ids = clean_rec ~in_usr_code catch env used_ids in
      TryCatch (p, b, c), used_ids

    | TryFinally (p, body, fin) ->
      let b, used_ids = clean_rec ~in_usr_code body env used_ids in
      let f, used_ids = clean_rec ~in_usr_code fin env used_ids in
    ...
    | Lambda (p, xs, body) ->
      ...
      let body, used_ids = clean_rec ~in_usr_code body env used_ids in
  ...
  | Some (_) ->
    ...
    let new_exp, new_ids = clean_rec ~in_usr_code:false exp IdMap.empty IdSet.empty in
    new_exp
